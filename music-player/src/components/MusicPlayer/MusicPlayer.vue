...
export default {
  ...
  watch: {
    song: function() {
      ...
      this.$http.get(this.urlMusic + this.song.songid).then(response => {
        this.url = response.body.data[0].url;
  ...
  data() {
    return {
      urlMusic: "http://115.159.46.181:3000/music/url?id=",
      url: null,
      volPos: {
        // 音量条的最大位置和当前位置,单位像素
        max: 80,
  ...
  methods: {
    ...
    btnMoveManager(type) {
      ...
      let t = {
        ...
        progressPos: function() {
          self.currLen = (
            self.progressPos.btn /
            self.progressPos.max *
            self.totalLen
          ).toFixed(4);
        }
      };
    ...
    btnUpManager(type) {
      ...
      let t = {
        ...
        progressPos: function() {
          self.$refs.audio.currentTime = (
            self.progressPos.btn /
            self.progressPos.max *
            self.$refs.audio.duration
          ).toFixed(4); // 设置音乐位置
    ...
    setProgress() {
      ...
      this.intervalId = setInterval(() => {
        ...
        this.progressPos.btn = parseFloat(
          (this.currLen / this.totalLen * this.progressPos.max).toFixed(2)
        );
    ...
    stopPass() {
      // 阻止按钮事件向下传递,否则会触发changePos
      return;
    },
    initMusic() {
      // 初始化音乐
      this.$refs.audio.addEventListener("loadedmetadata", () => {
        // 要在这个事件触发之后才能获得音乐的相关数据
...
$progressColor: rgba(225, 225, 225, 0.2);
$currentProgressColor: rgba(225, 225, 225, 0.7);
$volumeColor: rgba(225, 225, 225, 0.1);
$currentVolumeColor: rgba(225, 225, 225, 0.7);
$btnColor: rgba(225, 225, 225, 0.9);
